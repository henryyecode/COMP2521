...
void DLListBefore (DLList L, char *it)
{
	...
	DLListNode *nde = newDLListNode(it);
	
	// put on the before and after function
	if (L->nitems == 0) {
		L->curr = L->first = L->last = nde;
		L->nitems++;
		return;
	}
	...
	DLListNode *new = L->first;
	while(new->next != L->curr) {
	    new = new->next;
    }
...
void DLListAfter (DLList L, char *it)
{
	...
	DLListNode *new = L->last;
	while(new->prev != L->curr) {
	    new = new->prev;
    }
